...
클라이언트에서 최초 로그인 요청이 일어났을 때 서버에 세션 메모리 영역에 해당 세션ID를 하나 만들게 된다. 그리고 해당 세션ID만의 영역을 만들어 놓는다. 만약 ID PW가 정상이라면 해당 영역에 USER 정보를 저장한다. 이후 클라이언트에 세션ID를 리턴해주는데 웹브라우저로 요청해줬기 때문에 웹브라우저 프로그램에 쿠키라는 영역에 리턴받은 세션ID를 저장하게 된다.

그 이후로 새로운 요청이 일어나면 해당 쿠키의 세션ID를 들고간다. 서버는 해당 세션ID를 확인하는 방식으로 이루어진다.

이 방식의 안좋은 점은 서버가 하나일때는 괜찮지만 서버가 다중일 경우 서버마다 세션영역이 다르기 때문에 최초요청시 새로운 인증과정을 거쳐야한다.

우리가 ajax와 같은것을 쓰면 클라이언트가 javascript로 요청을 하게되는데 쿠키 정책은 동일 도메인에서는 요청을 할때 발동을 하게끔 되어있다.

a, b, c가 있다고 가정하고 a가 서버에 최초에 자바스크립트로 요청을할때 (www.naver.com) a의 ip주소인 210.10.10.5 로 요청을 하게되면 쿠키가 날라가지 않는다.

이때 강제로 쿠키를 포함해 요청을 보낼 수 있지만, 최근 대부분의 서버는 쿠키를 `HttpOnly`로 설정하여 HTTP 요청을 통해서만 쿠키를 전달받도록 합니다. 따라서 이러한 쿠키는 다른 방식의 접근 시 거부됩니다.

`HttpOnly` 설정을 false로 변경할 수는 있지만, 그렇게 하면 JavaScript로 쿠키에 무작위로 접근할 수 있게 되어 보안 문제가 발생할 수 있습니다.

또한 쿠키 방식을 쓰면 서버가 많아지면 많아질수록 확장성이 굉장히 떨어지게 된다.

그래서 Header에 Authorization이라는 key 값에 인증정보(ID, PW)를 넣고 가는 방식을 쓸 수 있다.

이것이 httpBasic 방식이다. 이렇게 요청하게되면 항상 요청시 ID와 PW를 달고 가게된다.

이렇게 요청을 하면 매번 요청이 ID와 PW를 달고가기 때문에 쿠키와 세션을 만들필요가 없다 매번 해당 정보를 담고가기 때문에

하지만 해당 정보는 암호화가 안되기 때문에 중간에 노출이 될 가능성이 있다.

노출이 안되게 하려면 서버를 https 서버를 써야한다.

하지만 우리가 사용하려는 방식은 Authorizaion에 토큰(ID, PW를 넣은)을 넣는 방식을 사용할 것 노출이 되더라도 해당 정보가 ID, PW값을 그대로 들고있는 것이 아니기 때문에 위험부담이 비교적 적다. 

이것을 Bearer 방식이라고 한다. JWT 토큰을 만들어서 해당 방식을 사용할 것